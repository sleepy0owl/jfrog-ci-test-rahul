...
```bash
jfrog-poc$ sam build --use-container
```
...
```bash
jfrog-poc$ sam logs -n StockCheckerFunction --stack-name jfrog-poc --tail
```
...
```bash
jfrog-poc$ pip install -r tests/requirements.txt --user
# unit test
jfrog-poc$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
jfrog-poc$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name jfrog-poc
```
